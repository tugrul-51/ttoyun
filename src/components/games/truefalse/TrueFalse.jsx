...
function getSeed(question = "", qi = 0) {
  const base = cleanText(question);
  return Array.from(base).reduce((sum, ch, index) => sum + ch.charCodeAt(0) * (index + 1), 0) + (qi + 1) * 97;
}

function buildAnswerClaim(optionText) {
  const answer = cleanText(optionText);
  return answer ? `Cevap: ${answer}` : "Cevap: -";
}
...
export default function TrueFalse({ q, qi, gqs, ans, hAns }) {
  ...
  const answerSeed = useMemo(() => getSeed(q?.q, qi || 0), [q?.q, qi]);

  const wrongOptionIndexes = useMemo(() => {
    if (!q?.o?.length) return [];
    return q.o
      .map((_, index) => index)
      .filter((index) => index !== q.a);
  }, [q]);

  const wrongOptionIndex = useMemo(() => {
    if (!wrongOptionIndexes.length) return q?.a === 0 ? 1 : 0;
    return wrongOptionIndexes[answerSeed % wrongOptionIndexes.length];
  }, [answerSeed, wrongOptionIndexes, q]);

  const statementIsTrue = useMemo(() => {
    if (!wrongOptionIndexes.length) return true;
    return answerSeed % 2 === 0;
  }, [answerSeed, wrongOptionIndexes.length]);

  const shownOptionIndex = statementIsTrue ? q?.a ?? 0 : wrongOptionIndex;
  const correctDecision = statementIsTrue ? "true" : "false";

  const statementText = useMemo(() => buildAnswerClaim(q?.o?.[shownOptionIndex] || ""), [q, shownOptionIndex]);
  ...
  function handleChoice(decisionKey) {
    if (ans !== null) return;
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event) => {
      ...
      if (["d", "y", "arrowleft", "arrowright", "1", "2"].includes(key)) {
        event.preventDefault();
      }

      if (key === "d" || key === "arrowleft" || key === "1") {
        handleChoice("true");
      }

      if (key === "y" || key === "arrowright" || key === "2") {
        handleChoice("false");
      }
    };
  ...
  const actualAnswerText = cleanText(q?.o?.[q?.a] || "");
  const shownAnswerText = cleanText(q?.o?.[shownOptionIndex] || "");
  ...
  const buttons = [
    {
      ...
      desc: "Gösterilen cevap gerçekten doğruysa bunu seç",
    },
    {
      key: "false",
      label: "YANLIŞ",
      ...
      desc: "Gösterilen cevap yanlış bir şıksa bunu seç",
    },
  ];
  ...
  return (
    ...
    >
      <style>{`
        ...
        .tf-question-scroll::-webkit-scrollbar {
          width: 8px;
        }

        .tf-question-scroll::-webkit-scrollbar-thumb {
          background: rgba(255,255,255,.20);
          border-radius: 999px;
        }

        .tf-statement-text::-webkit-scrollbar {
          width: 8px;
        }

        .tf-statement-text::-webkit-scrollbar-thumb {
          background: rgba(255,255,255,.20);
          border-radius: 999px;
        }
      ...
      >
        ...
        <div
          ...
          style={{
            ...
            border: "1px solid rgba(255,255,255,.10)",
            background:
              "linear-gradient(180deg, rgba(59,130,246,.14), rgba(14,18,32,.45))",
        ...
        >
          ...
          <div style={{ position: "relative", zIndex: 1 }}>
            ...
            <div
              className="tf-main-question tf-question-scroll"
              style={{
                ...
                letterSpacing: "-.02em",
                color: "#FFFFFF",
                textShadow: "0 4px 16px rgba(0,0,0,.24)",
                marginBottom: 14,
              }}
            >
              {cleanText(q?.q) || "Soru yükleniyor..."}
            </div>
            ...
            >
              ...
              >
                🧠 Önce soruyu oku, sonra aşağıdaki karar cümlesini değerlendir.
              </div>
              ...
              <div
                style={{
                  padding: "12px 14px",
                  borderRadius: 18,
                  background: ans === null ? "rgba(255,230,109,.10)" : selectedDecision === correctDecision ? "rgba(46,204,113,.12)" : "rgba(231,76,60,.12)",
                  border: ans === null ? "1px solid rgba(255,230,109,.18)" : selectedDecision === correctDecision ? "1px solid rgba(46,204,113,.18)" : "1px solid rgba(231,76,60,.18)",
                  color: "#FFF7D0",
                  fontSize: 14,
                  fontWeight: 800,
                }}
              >
                ⌨️ Kısayollar: D = Doğru, Y = Yanlış
              </div>
            </div>
          </div>
        </div>
          ...
          >
            ...
            <div style={{ position: "relative", zIndex: 1, marginBottom: 18 }}>
              ...
              >
                ...
                >
                  📌 Aşağıdaki kart, soruya verilmiş bir <strong style={{ color: '#fff' }}>cevap iddiası</strong> gösterir.
                </div>
                ...
                >
                  ⚖️ İddia doğruysa DOĞRU, diğer şıklardan biri gösterildiyse YANLIŞ seç.
                </div>
              </div>
              ...
              >
                ...
                >
                  ...
                  <div
                    className="tf-question-title tf-statement-text tf-statement-big"
                    style={{
                  ...
                  >
                    ...
                    >
                      🎯 Bazen gerçek cevap gösterilir, bazen de diğer 3 şıktan biri gösterilir.
                    </div>
                    ...
                    {ans !== null && (
                      <div
                        style={{
                          padding: '12px 14px',
                          borderRadius: 18,
                          background: selectedDecision === correctDecision ? 'rgba(46,204,113,.10)' : 'rgba(231,76,60,.10)',
                          border: selectedDecision === correctDecision ? '1px solid rgba(46,204,113,.18)' : '1px solid rgba(231,76,60,.18)',
                          color: '#fff',
                          fontSize: 13,
                          fontWeight: 800,
                          textAlign: 'left',
                      ...
                      >
                        {statementIsTrue
                          ? `Bu turda gösterilen cevap gerçekten doğruydu: ${shownAnswerText || actualAnswerText}`
                          : `Bu turda gösterilen cevap yanlıştı. Doğru cevap: ${actualAnswerText}`}
                      </div>
                    )}
                  </div>
                </div>
