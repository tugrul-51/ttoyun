  ...
  return (
    <div style={{ position: 'relative', overflow: 'hidden', borderRadius: compact ? 22 : 34, padding: compact ? 6 : 14, background: `linear-gradient(135deg, color-mix(in srgb, ${theme.accent} 16%, rgba(7,12,28,.94)), color-mix(in srgb, ${theme.accent2} 18%, rgba(7,12,28,.94)))`, border: '1px solid rgba(255,255,255,.10)', boxShadow: `0 24px 60px color-mix(in srgb, ${theme.accent} 18%, rgba(0,0,0,.38))`, minHeight: 0, height: '100%' }}>
      ...
      <div style={{ position: 'relative', display: 'grid', gap: compact ? 4 : 12, minHeight: 0, height: '100%', gridTemplateRows: compact ? 'auto 1fr' : 'auto auto 1fr' }}>
        <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: compact ? 6 : 12, padding: compact ? '6px 10px' : '14px 16px', borderRadius: compact ? 18 : 24, background: 'rgba(255,255,255,.07)', border: '1px solid rgba(255,255,255,.08)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: compact ? 10 : 14 }}>
            <div style={{ width: compact ? 34 : 64, height: compact ? 34 : 64, borderRadius: compact ? 14 : 22, display: 'grid', placeItems: 'center', fontSize: compact ? 16 : 30, background: `linear-gradient(135deg, ${theme.accent}, ${theme.accent2})`, boxShadow: `0 18px 30px color-mix(in srgb, ${theme.accent} 34%, transparent)` }}>{theme.icon}</div>
            <div>
              ...
              <div style={{ marginTop: 2, fontSize: compact ? 'clamp(12px, 1.8vw, 16px)' : 'clamp(22px, 3.6vw, 34px)', color: '#fff', fontWeight: 900 }}>{theme.title}</div>
              <div style={{ marginTop: compact ? 1 : 6, color: '#D6E6FF', lineHeight: 1.35, fontSize: compact ? 9 : 14 }}>{compact ? 'Sade sahne düzeni ve net soru alanı aktif.' : 'Çocuk dostu görseller, güçlü geri bildirim ve akıllı tahta uyumlu sahne düzeni aktif.'}</div>
            </div>
          </div>
          {!compact ? (
            <div style={{ minWidth: 150, alignSelf: 'stretch', padding: '12px 14px', borderRadius: 18, background: 'rgba(0,0,0,.16)', border: '1px solid rgba(255,255,255,.08)', display: 'grid', alignContent: 'center', gap: 4 }}>
              <div style={{ color: '#D5E7FF', fontWeight: 900, display: 'flex', alignItems: 'center', gap: 8, fontSize: 16 }}><span style={{ fontSize: 22 }}>{theme.mascot}</span> Sahne koçu</div>
              <div style={{ color: '#AEC4E3', fontSize: 13, lineHeight: 1.35 }}>Doğru cevaplarda parlayan sahne, yanlış cevapta öğretici geri bildirim ve özet ekranı hazır.</div>
            </div>
          ) : null}
        </div>
