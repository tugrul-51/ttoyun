...
<body>
  <div class="app">
    ...
    <div class="footer">T-T Eğitsel Oyunlar SCORM quiz paketi</div>
  </div>
  <script>
...
export function buildManifest(title, options = {}, filePaths = []) {
  const safe = String(title || 'T-T Eğitsel Oyunlar').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;').replace(/'/g, '&apos;');
...
export function sanitizeScormFileBase(value = 'T-T_Egitsel_Oyunlar') {
  return String(value || 'T-T_Egitsel_Oyunlar')
    .normalize('NFKD')
    .replace(/[\u0300-\u036f]/g, '')
    .replace(/[^a-zA-Z0-9-_]+/g, '_')
    .replace(/_+/g, '_')
    .replace(/^_|_$/g, '') || 'T-T_Egitsel_Oyunlar';
}
