...
function buildManifest(title = "T-T Eğitsel Oyunlar", files = []) {
...
function writeManifest(tempDistDir, title) {
  ...
  const manifestContent = buildManifest(title || "T-T Eğitsel Oyunlar", files);
...
function buildScormZipBuffer(data) {
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "tt-egitsel-oyunlar-scorm-"));
  const tempDistDir = path.join(tempRoot, "dist");

  copyDirRecursive(DIST_DIR, tempDistDir);
  injectScormData(tempDistDir, data);
  writeManifest(tempDistDir, data.topic || "T-T Eğitsel Oyunlar");
...
function toAsciiFilename(value) {
  return String(value || "T-T_Egitsel_Oyunlar_SCORM.zip")
    .normalize("NFKD")
    .replace(/[̀-ͯ]/g, "")
    .replace(/[^A-Za-z0-9._-]/g, "_")
    .replace(/_+/g, "_")
    .replace(/^_+|_+$/g, "") || "T-T_Egitsel_Oyunlar_SCORM.zip";
}
  ...
  if (req.method === "POST" && req.url === "/export-scorm") {
    ...
    req.on("end", () => {
      try {
        ...
        const safeName = (data.topic || "T-T_Egitsel_Oyunlar")
          .replace(/[^\p{L}\p{N}\-_ ]/gu, "")
          .trim()
          .replace(/\s+/g, "_");

        sendZip(res, `${safeName || "T-T_Egitsel_Oyunlar"}_SCORM.zip`, zipBuffer);
      } catch (error) {
